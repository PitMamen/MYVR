...
public class Utils {
	...
	public static Bitmap generateWLT(int percent, int width, int height,
		...
		canvas.drawText(perText, 124.0F, 36.0F, paint);
		return dstBmp;
	}
	...
	public static String getTime(long dateTaken) {
		...
		return calendar.get(Calendar.YEAR) + "\u5e74" + (1 + calendar.get(Calendar.MONTH)) + "\u6708";
	}
	...
	public static String[] getTimeInfo(long dateTaken) {
		...
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
		info[0] = String.valueOf(calendar.get(Calendar.YEAR));
		info[1] = String.valueOf(1 + calendar.get(Calendar.MONTH));
		info[2] = String.valueOf(calendar.get(Calendar.DATE));
		return info;
	}
	//TODO
